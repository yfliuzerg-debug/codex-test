...
import { takeExport } from "./_shared/export-store.mts";

function addSheet(wb: XLSX.WorkBook, name: string, rows: Record<string, any>[], widths: number[]) {
...
function formatOverlay(ws: XLSX.WorkSheet, rows: Record<string, any>[]) {
  if (!rows.length) return;
  const headers = Object.keys(rows[0]);
  const rateCol = headers.indexOf("费率");
  const moneyNames = new Set(["CP余额", "SO", "预测SO", "毛承载", "TTS已占用", "净承载", "预测承载", "计入可见承载"]);
  const moneyCols = headers.map((h, i) => (moneyNames.has(h) ? i : -1)).filter((i) => i >= 0);
...
export default async (req: Request, context: Context) => {
  ...
  const payload: any = await takeExport(context, token);
  if (!payload) {
    return new Response("下载结果已失效或已下载，请重新上传四个文件生成。", {
      status: 410,
  ...
  const overlayWs = addSheet(wb, "Overlay明细", overlay, [16, 24, 12, 15, 28, 12, 14, 34, 22, 16, 10, 14, 14, 10, 14, 14, 14, 14, 14, 42]);
  formatOverlay(overlayWs, overlay);
  ...
  wb.Props = {
    Title: "费用规划助手分析结果",
    Subject: "区域自主 Overlay 输入",
    Author: "费用规划助手",
    Comments: `生成时间 ${payload.generatedAt || ""}；版本 ${payload.version || ""}`,
  };
...
export const config: Config = { path: "/download-analysis" };
