...
(function(){
  ...
  function pivot(rows){var ms=S.month.size?Array.from(S.month).sort(cmp):uniq(rows.map(function(r){return r.month}));var m=new Map();rows.forEach(function(r){var gv=String(r[groupLevel]||""),k=[r.customer,r.customerCode,r.cp,r.channel,gv,r.sku].join("\\u241e"),g=m.get(k);if(!g){g={customer:r.customer,customerCode:r.customerCode,activity:r.activityLabel,channel:r.channel,group:gv,sku:r.skuLabel,values:{}};m.set(k,g)}g.values[r.month]=(g.values[r.month]||0)+(Number(r[metric])||0)});var out=Array.from(m.values()).sort(function(a,b){return cmp(a.customer,b.customer)||cmp(a.channel,b.channel)||cmp(a.group,b.group)||cmp(a.sku,b.sku)});return {months:ms,rows:out}}
