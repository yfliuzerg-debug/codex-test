...
const { analyzeFiles, BASELINE, labelMonth, money, months } = engine;
...
function renderResult(r: AnalysisResult, token = "", downloadError = "") {
  ...
  const note = `<div class="ok">历史扣重只扣同CP已存在计划；当前月同时保留“已实现HARD”和“整月预测上限”；下载文件保留客户×渠道×SKU×月份的Overlay明细。</div>${downloadError ? `<div class="warn">分析已完成，但下载缓存未建立：${esc(downloadError)}。请确认 Deno KV 已绑定到 App。</div>` : ""}`;
  const summary = `<div class="p"><h3>客户 × CP 汇总</h3><table><thead><tr><th>客户</th><th>CP</th><th>活动</th><th class="num">余额</th><th class="num">历史HARD</th><th class="num">本月已实现</th><th class="num">本月预测上限</th><th class="num">未来SOFT</th><th class="num">同CP扣减</th><th class="num">可见上限</th><th>状态</th></tr></thead><tbody>${r.matrix.map((x: any) => `<tr><td>${esc(x.name)}</td><td><b>${esc(x.cp.code)}</b></td><td class="activity">${esc(x.cp.desc)}</td><td class="num">${money(x.cp.balance)}</td><td class="num">${money(x.hard)}</td><td class="num">${money(x.currentActual)}</td><td class="num">${money(x.currentForecast)}</td><td class="num">${money(x.soft)}</td><td class="num">${money(x.deduct)}</td><td class="num"><b>${money(x.total)}</b></td><td>${x.unknown ? `<span class="tag">${x.unknown}个费率桶待确认</span>` : '<span class="tag">可测算</span>'}</td></tr>`).join("")}</tbody></table></div>`;
  const allMonths = [...new Set(r.active.flatMap((c: any) => months(c.start, c.end)))].sort();
  const detail = `<div class="p"><h3>逐月复核</h3><div class="muted">历史月与本月显示实际HARD；未来月显示SOFT。括号内是同CP既有计划扣减；净坑位低于500不计入汇总。</div><table><thead><tr><th>客户</th><th>CP</th><th class="activity">活动</th>${allMonths.map((m: string) => `<th class="num">${labelMonth(m)}</th>`).join("")}<th class="num">历史HARD</th><th class="num">含本月已实现</th></tr></thead><tbody>${r.matrix.map((x: any) => `<tr><td>${esc(x.name)}</td><td>${esc(x.cp.code)}</td><td class="activity">${esc(x.cp.desc)}</td>${allMonths.map((m: string) => { const v = x.per[m]; if (!v) return '<td class="num">—</td>'; const show = m <= cur ? v.actualNet : v.net, counted = m <= cur ? v.actualCounted : v.counted, ded = m <= cur ? v.actualDeduct : v.deduct, txt = show > 0 && show < 500 ? `<span class="low">${money(show)} &lt;500</span>` : money(counted); return `<td class="num">${txt}${ded > 0 ? `<div class="muted">(-${money(ded)})</div>` : ""}</td>`; }).join("")}<td class="num"><b>${money(x.hard)}</b></td><td class="num"><b>${money(x.hard + x.currentActual)}</b></td></tr>`).join("")}</tbody></table></div>`;
  return page("费用规划助手｜承载结果", filePanel(r) + cards + note + summary + detail, token);
}
...
async function handler(req: Request) {
  ...
  if (path === "/health") return Response.json({ ok: true, platform: "deno-deploy", time: new Date().toISOString() });
